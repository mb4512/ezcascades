...
from LAMMPS_AtomicDescriptors import Voxel_Descriptors
...
def main():
    ...
    parser.add_argument("-ngpu", "--numberofgpus", type=int, default=0, help="total number of GPUs to use for this simulation.")
...
WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING''' % temp)
    ...
    if ngpu > 0:
        lmp = lammps(cmdargs=["-sf", "gpu", "-pk", f"gpu {ngpu}"])
    else:
        lmp = lammps()
    ...
    if restartfile:
        ...
        if logdata.ndim == 1: # if just one line is in the file, make sure it still has dim 2 for parsing
            logdata = np.r_[[logdata]]
        dpadose = logdata[-1,1]
        ...
        elif initialtype == "restart":
            lmp.command('read_restart %s' % initial)
    ...
    def exclusion_radius (PKAenergy):
        ...
        elif potential.crystal == 'fcc':
            vmelt = nmelt * .25*alattice**3  # app. melt volume in Ang^3
        elif potential.crystal == 'hcp':
            vmelt = nmelt * (np.sqrt(3)/4) * c_over_a * alattice ** 3  # app. melt volume in Ang^3
        else:
            vmelt = nmelt * alattice ** 3
    ...
    ###########################
    # BEGIN TEST IMAGE CONSISTENCY
    ###########################

    periodic_image_cutoff = 5

    # If any of the images are greater than 5 reset everything to zero as something must have went wrong

    rst_rank = False

    img = lmp.numpy.extract_atom('image')
    nlocal = lmp.extract_global('nlocal', dtype=LAMMPS_INT)

    for i in range(nlocal):
        img_decode = lmp.decode_image_flags(img[i])
        for _img in img_decode:
            if abs(_img) > periodic_image_cutoff:
                rst_rank = True
                print(f'Periodic Image found {_img}')
                sys.stdout.flush()
                break
        if rst_rank:
            break
    ...
    rst = comm.allreduce(rst_rank, op=MPI.LOR)

    if rst:
        announce(f'Found atoms with a Periodic Image Greater than {periodic_image_cutoff}')
        lmp.command('set atom * image 0 0 0')

    ###########################
    # END TEST IMAGE CONSISTENCY
    ###########################
    ...
    try:
        ...
        for cloop in range(1, int(1e6)):
            ...
            announce('Atoms set to averaged co-ordinates to compute descriptors')
            ...
            announce('Atoms set to momentary co-ordinates to return to cascade simulations')
